...
const stopButton = /** @type {HTMLButtonElement} */ (document.getElementById("stopButton"));
const recordMicCheckbox = /** @type {HTMLInputElement} */ (document.getElementById("record-mic"));
const labelElement = /** @type {HTMLInputElement} */ (document.getElementById("record-mic-label"));
const logElement = /** @type {HTMLPreElement} */ (document.getElementById("log"));
/** @type {number} */
let recordingStart;
...
/**
 * @param {{recordingStart: number, chunks: Blob[]}} param 
 */
async function onRecordFinish({  recordingStart, chunks }) {
    window.onbeforeunload = () => { };
    (/** @type {MediaStream} */  (preview.srcObject))?.getTracks()?.forEach((track) => track.stop());
...
async function main() {
    ...
    const intervallId = setInterval(() => {
        const videoTracks = (/** @type {MediaStream} */  (preview.srcObject))?.getVideoTracks();
    ...
    const chunks = [];
    // @ts-ignore
    await record(preview.captureStream(), chunks);
    await onRecordFinish({ recordingStart, chunks });
}
